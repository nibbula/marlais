...
static int
read_eval_print(FILE * f, int dbg_lvl, int bind_constant_p)
{
  int x, vals;
  Object obj;
...
#define COMMON_DYLAN_LIB_DIR "common"
  ...
  if(execute) {
    ...
    read_eval_print(stdin, debugging_output, 0);
    if(!stay) exit(0);
    optind++;
  }
  ...
  while (read_eval_print(stdin, debugging_output, 1)) {
    cache_env = the_env;
    current_prompt = prompt;
  }
