...
define_class_eval (Object form)
{
  ...
  if (EMPTYLISTP (CDR (tmp_form))) {
    error ("malformed define-class (no arguments)", form, NULL);
  }
...
static Object
define_method_eval_helper (Object form)
{
...
{
  return define_method_eval_helper(form);
}

static Object
define_function_eval_helper (Object form)
{
  Object name, params, body, method;

  check_function_syntax(form, &name, &params, "define-function");
  body = CDR (CDR (CDR (form)));
  method = make_method (name, params, body, the_env, 0);
  add_top_level_binding (name, method, 1);
  return (name);
}
...
{
  return define_function_eval_helper(form);
}
