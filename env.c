...
#define TOP_LEVEL_SIZE 16384 /* @@@ 1024 */
...
static struct module_binding * module_binding_internal (Object module_name);
...
initialize_namespace (Object owner)
{
  ...
  frame->owner = owner;
  frame->bindings =
    (struct binding **) checking_malloc (TOP_LEVEL_SIZE *
...
/*
** This hack exists because we don't have macros and I can't think of
** another way to make the FFI work nicely for now.
*/
Object
add_top_level(Object sym, Object val)
{
  if (PAIRP(sym) && KEYWORDP(CAR(sym)))
    CAR(sym) = keyword_to_symbol(CAR(sym));

  add_top_lvl_binding1(sym, val, 0, 0);
  return val;
}
...
#endif
    {"%add-top-level-binding", prim_2, add_top_level},
};
...
symbol_binding (Object sym)
{
	...
	frame = frame->next;
	if (!frame)
	  /* <pcb> I/'ve observed this to be nil in a special case. */
	  break;
    }
...
static struct module_binding *
module_binding_internal (Object module_name)
{
...
{
    struct module_binding * binding = module_binding_internal(module_name);
    if (!binding)
...
struct module_binding *
find_module(Object module_name)
{
  struct module_binding * result;

  result = module_binding_internal(module_name);
  if (!result)
...
#else
		     (table_element (imports_table, binding->sym, false_object)
#endif
		      != false_object))) {
