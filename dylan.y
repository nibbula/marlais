%{
/*
** dylan.y -- dylan phrase grammar from 5/12/94 interim report 
*/
...
{
    char line_str[20];
    sprintf (line_str, " [line #%d char %d]", yylineno, yybyte);
    warning (s,
...
#ifdef DEBUG_OPTIMIZE_SPECIALIZERS
    debug ("Got symtab_insert_bindings", bindings, NULL);
#endif
...
symtab_push_parameters (Object parameters)
{
    ...
    symtab = cons (make_table (DEFAULT_TABLE_SIZE), symtab);
#ifdef DEBUG_OPTIMIZE_SPECIALIZERS
    debug ("Got symtab_push_parameters", parameters, NULL);
#endif
