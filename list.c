...
Object
list_merge(Object a, Object b, Object compare)
{
  Object merged = make_empty_list();

  while (a && b)
  {
    if (apply(compare, listem(a, b, 0)) != false_object)
    {
      merged = cons(merged, a);
      c->next = a;
      a->prev = (c == &merged) ? 0 : c; // @@@ get this out of the loop
      c = a;
      a = a->next;
    }
    else
    {
      c->next = b;
      b->prev = (c == &merged) ? 0 : c; // @@@ get this out of the loop
      c = b;
      b = b->next;
    }
  }
...
// Merge sort with a comparison function
template <class T> template <class Compar>
ft_List<T>::Node *
ft_List<T>::merge_sort_with_compare(ft_List<T>::Node * c, Compar compare)
{
  if (!c->next)
    return c;

  Node * a = c;			// Save the head
  Node * b = c->next;

  // Find something like the middle
  while (b && b->next)
  {
    c = c->next;
    b = b->next->next;		// Two for every one
  }
  ...
  // Sort left and right recursivly, then merge
  return merge_with_compare(merge_sort_with_compare(a, compare),
			    merge_sort_with_compare(b, compare), compare);
}

template <class T> template <class Compar>
void
ft_List<T>::sort(Compar compare)
{
  head = merge_sort_with_compare(head, compare);

  // @@@ Updating the tail probably should be done in the merge (special case?)
  // O(n + (n log n))
  Node * n = head;
  while (n->next)
    n = n->next;
